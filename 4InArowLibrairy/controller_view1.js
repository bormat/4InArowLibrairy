...
app.controller('View1Ctrl',function ($scope,$timeout,keyboard,$cookieStore) {
		...
		/*function disableDrag(){
			var pions=$("#applet span");
			pions.on("dragstart",function () { return false; });
			return true;
		}	
		whenReady("$('#applet span')[41]",disableDrag);*/
	...
	var scollOption = function(){
		...
		var lg=currentY - lastY;
		if ( Math.abs( lg ) > 1  ){ 
			var init= $(window).scrollTop();
			//var deltaX = (originX-currentX);
			if ($scope.scroll){
				$(window).scrollTop(init-lg);
				lastY=currentY;
			}
